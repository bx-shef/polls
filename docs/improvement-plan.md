...
> refresh_token → сверка authoritative `member_id`; 403 mismatch/4xx, 503 инфра) + вызов в
> install-хендлере ДО `handleInstall` (ротация → `verifiedAuth` во все пути). Осталось: **живой smoke**
> установки на портале (рефреш свежевыданного токена; двойная доставка page+event).
