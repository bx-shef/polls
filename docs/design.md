...
**Композиция:** шапка (вордмарк + «Опрос '26») → крупный заголовок (Unbounded) →
...
const items = computed(() => q.options.map((o, i) => ({
  label: o.label, value: o.id, key: i < 9 ? i + 1 : null, other: !!o.other
})))
...
### 5.5. «Другое — свой вариант» → `B24Input` со счётчиком
...
<template>
  <B24Page>
    ...
    <B24PageBody>
      ...
      <B24Card v-for="q in questions" :key="q.id" :title="q.title" class="mb-4">
