...
| `SURVEY_KEY_DEAL` / `SURVEY_KEY_TASK` / … | какой опрос запускать по сделке/задаче из виджета (ключ опроса); по умолчанию `SURVEY_KEY_DEFAULT` | нет (дефолт `csat_postdeal`) |
