 ...
 *
 * Здесь то же условие, но с именем виновника. Имена переменных — не секрет: они лежат
 * в `.env.example` и `deploy/README.md`. Секрет — значения, и их тут нет ни в одном виде,
 * даже длиной: длина ключа сужает перебор.
 *
...
/** Имя переменной окружения ровно в том виде, в каком её правят в `.env` на хосте. */
 ...
 *
 * Порядок фиксирован и совпадает с порядком в `.env.example` — журнал и `/api/health`
 * читает человек, и «всегда в одном порядке» здесь дороже алфавита.
 */
