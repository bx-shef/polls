...
export interface FramePass {
   ...
   * Грант портала.
   *
   * ⚠ Читаем его ТОЛЬКО в мастере установки — но отдаёт его SDK ВСЕГДА, в каждой фреймовой
   * сессии любого сотрудника, пока фрейм авторизован (`#refreshId = data.REFRESH_ID`
   * ставится при инициализации и возвращается из `getAuthData()` без условий). Здесь стояло
   * «есть только при установке» — неправда, и неправда опасная: из неё следовало бы, что
   * приёмник установки видит грант только от администратора. Он видит его от кого угодно,
   * поэтому права проверяются отдельно. Нашла панель ревью PR #27.
   */
  refreshToken: string
