...
<template>
  <B24Card variant="outline" class="w-full max-w-xl">
    <div class="flex flex-col items-center gap-5 py-2 text-center">
      ...
      <p v-if="intro.lead" class="max-w-md text-base text-gray-500 dark:text-gray-400">
        {{ intro.lead }}
      </p>
      ...
      <p v-if="intro.count" class="text-sm text-gray-500 dark:text-gray-400">{{ intro.count }}</p>
    </div>
  </B24Card>
</template>
