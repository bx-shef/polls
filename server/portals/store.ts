...
/**
 * Сколько порталов сейчас под отсчётом.
 *
 * Числитель предохранителя по доле флота и то же число, что показывает `make prod-portals`.
 * ⚠ В анонимный `/api/health` оно НЕ отдаётся: на флоте из одного портала переход 0→1 означал
 * бы «у этого единственного клиента грант помечен мёртвым» — в реальном времени и всем
 * желающим. Нашла повторная панель ревью PR #34.
 */
