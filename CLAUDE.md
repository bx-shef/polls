...
(NPS/CSAT/распределение/тренд NPS) — серверный агрегат через `domain/aggregate` над общим стором; вопросы
берутся по МЕТРИКЕ из текущей версии (не хардкод seed-ключей), малые N подавлены на уровне
...
бакету (`minN` в `npsTrend`, тот же порог) — анонимность по месяцу. Типы метрик клиент берёт из
