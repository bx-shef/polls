...
- **Срок доступности ссылки — `invitationPolicy.linkTtlSeconds`** (Фаза F) — окно [5 мин, 5 дней]
  ...
  единая точка контроля; `peek`/`consume` уже чистят протухшее по сроку, так что 403 после истечения даётся
  без нового кода. Не задан → дефолт стора (30 дней, back-compat со старыми опросами); вне диапазона →
  parse-ошибка на границе (`compile`/PgStore-read), а не тихий кламп.
