...
test('на экране ошибки в заголовке НЕ печатается ключ из адреса', async ({ page }) => {
  // Без скриншота. Раньше `<h1>` показывал сырой параметр маршрута, то есть ссылкой
  // `/d/<любой текст>` можно было нарисовать чужой текст заголовком на нашей странице.
  await page.goto('/d/текст-которого-нет-в-базе', { waitUntil: 'networkidle' })
  await expect(page.getByRole('heading', { level: 1 })).toHaveText('Результаты опроса')
})
