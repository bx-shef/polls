...
| 5 | Дашборд результатов (контур B): `/d/:key` NPS/CSAT/распределение/тренд NPS/срез по услугам поверх `domain/aggregate` | 1, 3 | 🔶 KPI + тренд + срез по услугам под гейтом (dev); срезы клиент/направление + KPI ответственных (`kpiByResponsible`; нужна денормализация имён в `CrmContext`) → дальше; auth-гейтинг → #47 |
