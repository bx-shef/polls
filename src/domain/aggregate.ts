...
/** Распределение после подавления малых ячеек: что показываем и СКОЛЬКО ячеек скрыто. */
export interface SuppressedDistribution {
  items: DistributionBin[]
  /**
   * Число скрытых ячеек. Публикуется намеренно: график без пометки читается как полный, и человек
   * сделает вывод по неполной картине, не зная об этом. Сами скрытые метки и их счётчики наружу НЕ
   * идут — иначе подавление было бы декоративным.
   */
  hiddenBins: number
}
 ...
 *
 * ⚠️ **Комплементарное подавление**: если скрыть удалось ровно ОДНУ ячейку, скрывается ещё и самая
 * маленькая из оставшихся. Иначе подавление ничего не даёт: у обязательного вопроса с одиночным
 * выбором сумма ячеек равна общему N, который мы публикуем, — и скрытая ячейка восстанавливается
 * вычитанием. Это стандартное правило раскрытия статистики, а не перестраховка: без него «скрыто: 1»
 * работает подсказкой, куда смотреть, вместо защиты.
 *
 * ⚠️ Итог может оказаться ПУСТЫМ (два варианта, у одного — единица): показать второй значило бы
 * назвать первый. Пустое распределение — правильный ответ, а не сбой.
 *
 * Порядок входных ячеек сохраняется — сортирует вызывающий.
 */
export function suppressSmallBins(
  items: DistributionBin[],
  minN: number = ANONYMITY_THRESHOLD
): SuppressedDistribution {
  const shown = items.filter((i) => meetsAnonymity(i.count, minN))
  const hiddenBins = items.length - shown.length
  if (hiddenBins !== 1) return { items: shown, hiddenBins }

  // Показанных нет вовсе (ячейка единственная, и та мала) — добирать нечего.
  if (shown.length === 0) return { items: shown, hiddenBins }

  // Скрыта ровно одна ячейка — добираем самую маленькую из показанных.
  let smallest = 0
  for (let i = 1; i < shown.length; i++) {
    if (shown[i]!.count < shown[smallest]!.count) smallest = i
  }
  shown.splice(smallest, 1)
  return { items: shown, hiddenBins: hiddenBins + 1 }
}
