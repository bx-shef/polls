...
Полная инструкция и секреты — [карта проекта](./docs/project-map.md), §Деплой и эксплуатация;
чек-лист живого прогона на портале — там же, §Ручной прогон режима приложения Bitrix24.
...
Ровно три файла — по одному на вопрос:

- [**Как работает сервис**](./docs/process.md) — процесс по шагам простыми словами: установка
  приложения на портал → настройка опроса → запуск по стадии сделки → прохождение → возврат
  результата в Bitrix24 и аналитика.
- [**Карта проекта**](./docs/project-map.md) — из каких частей состоит проект и в каком каждая
  состоянии (✅ сделано · 🔎 проверено вживую · ⏳ отложено): ядро, приложение, данные, интеграция,
  безопасность, деплой, проверки качества, решения, словарик.
- [**Что делаем дальше**](./docs/roadmap.md) — планы от ближайшего запуска до «потом-потом».

Плюс [обезличенный шаблон схемы](./docs/reference/survey-schema.template.json) — структурный скелет
опроса (типы, флаги, блоки) без оригинальных формулировок.

---
*Последнее ревью: 2026-07-29.*
