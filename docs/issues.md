...
| NEW | Админ-UI создания/редактирования опросов (визуальный конструктор поверх `SurveyDraft`/`publish`) | app/api | предложен (номер присвоит GitHub) | сейчас опросы — в коде (`demo/seed.ts`)/через `store.publish`; ядро версионирования готово. См. `survey-management.md` |
