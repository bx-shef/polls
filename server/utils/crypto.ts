...
/**
 * Есть ли рабочий ключ шифрования.
 *
 * Нужна ровно для того, чтобы проверить это ДО первого исходящего запроса. Обмен токена
 * вращает грант, и падение на отсутствующем ключе после обмена сжигает единственный
 * `refresh_token` администратора; событие установки портал не повторяет.
 */
