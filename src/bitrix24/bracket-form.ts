 ...
 *
 * Один уровень вложенности (достаточно для событий B24: `auth[x]`/`data[x]`). Идемпотентно на уже
 * вложенном входе (JSON-тело: ключ без скобок → значение-объект проходит как есть). Гард от
 * prototype-pollution: ключи `__proto__`/`prototype`/`constructor` (родитель или потомок) отбрасываются.
 */
