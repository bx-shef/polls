 ...
 * не отдаётся наружу.
 */
// ⚠ `trim()` обязателен, и не ради красоты. Проверка готовности (`server/utils/readiness.ts`)
// считала пустой строку из одних пробелов, а сюда значение уходило как есть — то есть
// `B24_CLIENT_SECRET=секрет␠` проходил бы проверку и отправлялся на сервер авторизации
// с хвостом. Битрикс24 отвечал бы отказом, а мастер показывал «приложение устанавливает
// не администратор» — диагноз, не имеющий отношения к причине. Обрезаем в одном месте,
// чтобы проверяющий и потребитель видели одну и ту же строку. Нашла панель ревью PR #28.
export const b24ClientId = (): string => (process.env.B24_CLIENT_ID ?? '').trim()
export const b24ClientSecret = (): string => (process.env.B24_CLIENT_SECRET ?? '').trim()
