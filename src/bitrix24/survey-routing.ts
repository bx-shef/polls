...
/**
 * Маршрутизация «по какой сущности — какой опрос запускать» (ручной виджет сделки и будущий
 * автотриггер). Раньше эндпоинты хардкодили `csat_postdeal` для всех — теперь ключ опроса берётся из
 * конфигурации портала (env), с дефолтом. Это позволяет, например, по лиду запускать опрос
 * квалификации, а по сделке — постпродажный NPS. Чистая функция, конфиг инжектируется.
 *
 * Полноценный UI-маппинг (`entityType → surveyKey` в админке/БД) — отдельный шаг (см. `docs/roadmap.md`);
 ...
 * (`SURVEY_KEY_DEAL`, `SURVEY_KEY_LEAD`, …). Неизвестные/пустые — пропускаются. Источник env
