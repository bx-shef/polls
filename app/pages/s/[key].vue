 ...
 * (#49): ответ обоих запросов зависит от портала, а портал выводится из токена.
 *
 * ⚠️ Токен из ключа ПОПАДАЕТ в разметку: ключ `useAsyncData` — это имя поля в `payload.data`, а payload
 * сериализуется в `__NUXT_DATA__`. Допустимо ровно потому, что он и так стоит в адресной строке ЭТОЙ
 * ЖЕ страницы (плюс `no-store` и `noindex`), — а не потому, что «в разметку не попадает»: так было
 * написано в первой редакции, и это неверно. Секрет, которого нет в URL, в ключ класть нельзя.
 */
