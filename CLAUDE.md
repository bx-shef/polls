...
секрета И не-dev → 503 (fail-closed, чтобы имена клиентов/`responsibleName`-PII не утекли). Dev/гейт
открыты по `DASHBOARD_DEV_OPEN=1` (собранный сервер бежит как production). Осталось по #47: handshake
Bitrix24 app-фрейма (минт сессии) + tenant-фильтрация стора по `portalId` (PgStore, #49); rate-limit → #49.
