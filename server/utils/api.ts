...
/**
 * Побочные действия после записи ответа: закрыть дело-приглашение в таймлайне (#177) и положить
 * результат опроса в карточку сделки (#18).
 *
 * Динамический импорт разрывает цикл `api.ts → portal-deps.ts → api.ts` (сборке зависимостей нужны
 * `usePortalDb`/`usePortalId`/`logger` отсюда же). Он же оставляет путь без БД нетронутым: модули
 * просто не грузятся.
 */
function onAnsweredHookFor(portalId?: number): (info: AnsweredInfo) => Promise<void> {
  return async (info: AnsweredInfo): Promise<void> => {
    ...
    // клиенту экране. Клиент портала у них ОДИН: `livePortalDeps` помнит резолв на набор зависимостей,
    // а поверх стоит очередь по порталу — то есть параллельность стоит одного лишнего промиса, а не
    // второго обращения к базе и второго рефреша токена.
