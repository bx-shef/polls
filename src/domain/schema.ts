...
export const invitationPolicySchema = z.object({
   ...
   * (вне диапазона → parse-ошибка на границе `compile`/PgStore-read, а не тихий кламп). После истечения
   * ссылка не открывается, а `POST /api/submit` → 403: и `peek` (предпросмотр по токену), и `consume`
   * (запись ответа) в `MemoryInvitationStore` вычищают протухшие токены по `exp`. **Необязателен:**
   * не задан → падает на дефолт стора приглашений (30 дней, back-compat со старыми опросами без поля);
