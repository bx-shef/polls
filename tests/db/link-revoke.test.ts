...
describe.skipIf(!enabled)('отзыв ссылки', () => {
  ...
  it('уносит имена людей вместе с отзывом', async () => {
    ...
    // больше не покажет — значит хранить имена дальше нечем оправдать. То же правило действует
    // для пройденных (`markCompleted`) и для истёкших (`forgetExpiredLinkHeaders`); последнее —
    // с 25.09, до инвентаризации базы этот комментарий обещал то, чего не было.
    const id = await seed(portalId, 54)
