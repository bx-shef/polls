 ...
 *
 * ⚠ Предохранителя по доле, как у `purgeDeadPortals`, здесь НЕТ, и это осознанно.
 * Там доля флота отличает исход клиентов от нашей поломки. Здесь отличать нечего: строка
 * попадает под удаление, только пробыв `failed` тридцать суток, и массовое срабатывание
 * означает настоящий массовый отказ доставки — который как раз и надо убрать, а не задержать.
 * Видимость обеспечивает построчная запись: разом стёртая сотня даст сотню строк в журнале.
 */
export async function purgeExpiredAnswers(now: Date, limit = 50): Promise<number> {
  const db = getDb()
  ...
  // а стереть можно только раз. С потолком беда идёт по пятьдесят строк в минуту, каждая
  // со своей строкой в журнале, то есть заметна задолго до того, как станет непоправимой.
  ...
  const rows = await db
    .delete(schema.inbox)
    .where(inArray(schema.inbox.id, due))
    .returning({
      id: schema.inbox.id,
      portalId: schema.inbox.portalId,
  ...
  for (const row of rows) {
    logger.warn(
      { portalId: row.portalId, receivedAt: row.receivedAt.toISOString(), reason: row.lastError ?? 'неизвестно' },
