...
```bash
pnpm install
pnpm verify      # печатает агрегаты на 4 уровнях по демо-данным
pnpm test        # 40 тестов (движок, версионирование, агрегация)
...
scripts/       verify.ts
test/          vitest — 40 тестов, покрытие доменного ядра 100% строк
