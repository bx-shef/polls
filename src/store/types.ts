...
export interface DashboardAggregates {
  ...
  versions: number[]
  /**
   * Фильтр, который РЕАЛЬНО применён: запрошенный `versionNo`, если такая версия существует, иначе
   * `null` (посчитано по всем версиям).
   *
   * ⚠️ Решает хранилище, а не вызывающий, и это не педантизм. Список версий известен только после
   * запроса, поэтому вид, проверявший его у себя, был вынужден при кривом `?version=` идти в
   * хранилище ВТОРОЙ раз — то есть опечатка в ссылке удваивала нагрузку. Здесь ответ уже есть.
   */
  version: number | null
  nps: NpsSummary | null
 ...
 * `bitrix24/portal` (исключение плейсхолдера из keep-alive и резолва арендатора). Импорт
 * `bitrix24 → store/bootstrap` разворачивал бы зависимость слоёв (бутстрап тянет `demo/seed` со
 * всем демо-датасетом) и подводил бы к циклу.
 */
