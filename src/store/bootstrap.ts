...
/**
 * Портал по умолчанию — ПЛЕЙСХОЛДЕР (связки с Bitrix ещё нет)?
 *
 * ⚠️ Ответ решает, можно ли сеять демо-данные. Пока портал был один, вопрос не стоял; с
 * мультитенантом (#47/#49) «портал по умолчанию» — это самый ранний НАСТОЯЩИЙ портал, то есть чей-то
 * боевой тенант, и разница перестала быть теоретической (см. {@link seedDemoIfEmpty}).
 */
 ...
 * {@link isPlaceholderPortal}), и это несущее условие, а не гигиена. С мультитенантом порталом по
 * умолчанию становится самый ранний НАСТОЯЩИЙ, а обычное удаление приложения (`CLEAN=1`) сносит его
 * строку и сбрасывает кэш стора — то есть следующий же запрос выбирает порталом по умолчанию
 * СЛЕДУЮЩЕГО арендатора. У него демо-опроса нет, и без гейта boot опубликовал бы ему две версии
 * анкеты и дюжину выдуманных ответов с чужими названиями компаний и ФИО ответственных. Дальше это
 * уже не косметика: ключ демо совпадает с `DEFAULT_SURVEY_KEY`, по которому виджет и робот выписывают
 * приглашения, — реальному клиенту ушла бы ссылка на демо-анкету, а дашборд смешал бы выдуманные
 * ответы с настоящими. Ни ошибки, ни отличия от нормы — только `store_seeded` в логе.
 *
 * ⚠️ Остаток, который этим гейтом НЕ закрывается: плейсхолдер вместе с демо-данными ПРИСВАИВАЕТСЯ
 * первым установившим порталом (`SaveTokensOpts.adoptLocal`, #171) — то есть первый арендатор
 * получает демо-опрос в свои данные. Это поведение #171/#183, а не мультитенанта; собственный
 * неприсваиваемый демо-портал заведён отдельной задачей.
 */
