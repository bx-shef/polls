...
export interface ApiDeps {
  ...
  /**
   * Что сделать ПОСЛЕ успешно записанного ответа — сегодня закрыть дело-приглашение в таймлайне
   * сделки ([#177](https://github.com/bx-shef/polls/issues/177)).
   *
   * ⚠️ Инжектируется, потому что ядро про REST не знает и знать не должно: `submit` — публичный
   * роут без портального клиента, а сходить в CRM надо. Реализация живёт в Nitro-слое.
   *
   * ⚠️ **Ответ клиента дороже отметки в таймлайне.** Хук вызывается ПОСЛЕ записи, его отказ наружу
   * НЕ пробрасывается и на ответ 200 не влияет: заставить человека заполнять анкету заново из-за
   * недоступности портала — худшее, что тут можно сделать. Но и молча отказ не теряется — уходит в
   * `onError`, то есть диагностика остаётся там же, где и остальные внутренние сбои.
   */
  ...
  return {
    ...
    async submit({ ip, body }: SubmitInput): Promise<ApiResult> {
      ...
      try {
        ...
        // Побочные действия — ПОСЛЕ записи и расхода, и они не могут испортить ответ клиенту.
        // `onAnswered` заявлен возвращающим промис: без `await` его отказ стал бы unhandled rejection,
        // а `void` без обработчика — тихой потерей диагностики.
        if (deps.onAnswered) {
          // ⚠️ `Promise.resolve().then(...)`, а не `hook(...).catch(...)`: контракт хука —
          // `=> Promise<void>`, но СИНХРОННЫЙ throw реализации мимо `.catch` пролетит, и клиент
          // получит 500 «не удалось сохранить» по УЖЕ записанному ответу. Сегодняшняя реализация
          // асинхронная и не стреляет — но контракт этого не гарантирует, а падение проявится у
          // следующей.
