...
export default defineConfig({
  ...
  expect: {
    toHaveScreenshot: {
      ...
      //
      // Было 0.002 (~0.2%, то есть ~2000 пикселей на десктопе) — и этого хватало, чтобы ЦЕЛЫЙ
      // новый элемент интерфейса прошёл мимо гейта: виджет отзывов 👍/👎 приехал в #141, занял
      // на десктопном дашборде около 0.15% кадра, тест остался зелёным, а эталон так и остался
      // без виджета. То есть «эталон» перестал описывать экран, и никто об этом не узнал.
      //
      // 0.0005 (~0.05%, ~500 пикселей на десктопе) оставляет запас на субпиксельный антиалиасинг
      // (среда закреплена: один и тот же chromium, системные шрифты, анимации выключены — по факту
      // прогоны байт-в-байт), но строку текста или новый блок уже не прощает. Если гейт начнёт
      // мигать без правок UI — поднимать порог обратно НЕ надо: сначала найти источник недетерминизма.
