...
- **Развёрнут вживую:** `polls.bx-shef.by` (Docker + nginx-proxy + TLS + PostgreSQL,
  авто-CD merge→GHCR→watchtower). **Live-verified — read-путь** (вебхук); install/handshake **код готов,
  ждут живого smoke** (см. §Статус).
...
3. **#17** — авто-триггер `ONCRMDEALUPDATE`: эндпоинт `/api/b24/deal-update` + `event.bind` при install
   ...
   **дедуп/детекция перехода** (`ONCRMDEALUPDATE` бьёт на ЛЮБОЙ апдейт, не на смену стадии → **блокер
   перед включением вживую**, дедуп по `dealId+surveyKey+stage`); **живой smoke**; доставка ссылки
   (email/SMS); робот-эндпоинт `/api/b24/robot` (робот пока не регистрируется).
...
- **`bitrix24/`** — интеграция (server-only):
  ...
  - `deal-event.ts` / `entity-event.ts` — триггер `ONCRMDEALUPDATE` + мульти-сущность
    ...
    `mapProductRows`, `entityToCrmContext`); `bracket-form.ts` — разбор form-urlencoded событий
...
стор по `DATABASE_URL` — PgStore (прод, миграции + `setPortalResolver`) / MemoryStore+seed (dev)) + роуты `server/api/`:
...
- **Установка** `/api/b24/install` (два формата: install-страница + событие `ONAPPINSTALL`):
  ...
  **503** транзиент; рефреш обёрнут `AbortSignal.timeout` 10с) → `applyVerifiedTokens` (ротированный
  ...
  `isAllowedPortalDomain` ДО REST → `handleInstall` (сохранить токены + `event.bind ONCRMDEALUPDATE` →
  `/api/b24/deal-update` (#17) + 2 плейсмента; каждая регистрация толерантна к ошибке; робот
  `bizproc.robot.add` НЕ регистрируется — эндпоинт `/api/b24/robot` не реализован). Rate-limit
  `allowB24Install` 20/60с ДО рефреша (анти-амплификация). Идемпотентность двойной доставки —
  `decideInstallDoubleDispatch`.
...
- **`invitationPolicy`/презентация — version-frozen** (#21/#25) — ответ интерпретируется по версии, на
  которой собран.
- **Срок доступности ссылки — `invitationPolicy.linkTtlSeconds`** (Фаза F) — окно [5 мин, 5 дней]
  (`[300, 432000]` сек), настраивается на каждый опрос, заморожено с версией. Проводится в TTL токена
  приглашения **на выписке** (`handleDealTrigger`/`createSurveyInvitation`), а не хранится отдельным полем —
  единая точка контроля. Живой гейт истечения — `consume` на `POST /api/submit` → 403 (без нового кода);
  `peek`/`consume` чистят протухшее по `exp` и на уровне стора (предпросмотр по токену — когда HTTP-роут его
  задействует; сейчас `peek` не подключён). Не задан → дефолт стора (30 дней) — back-compat со старыми опросами
  И новыми версиями без явного поля: **потолок 5 дней валидирует лишь ЗАДАННОЕ значение, глобальным
  инвариантом не является** (UI-пикер должен дефолтить в диапазон). Вне диапазона → parse-ошибка на границе
  (`compile`/PgStore-read), а не тихий кламп.
- **Доставка через настраиваемое дело — чистый билдер** (`bitrix24/activity.ts`, Фаза F) — параметры
  `crm.activity.configurable.add` строит framework-agnostic `buildSurveyInviteActivity` (тестируется без
  портала), REST шлёт тонкая `activityConfigurableAdd`. Структура сверена с REST-контрактом Bitrix И с
  **live-verified** соседом `ai-price-import`: `body.logo` ОБЯЗАТЕЛЕН (иначе 400 «Поле logo в BodyDto…»),
  `redirect.uri` — только same-portal относительный путь (SSRF), блоков 1..20 / кнопок ≤2. Внешняя ссылка на
  анкету — ТЕКСТОМ (не `redirect`, тот навигирует внутри портала); кнопка «Отправить приглашение» —
  `openRestApp` (виджет ловит через `bindLayoutEventCallback`). Постинг/виджет/живой smoke — отдельные срезы.
- **Триггер завершения — сводка ответа → таймлайн** (`domain/result-summary.ts` + `activity.ts`, #18) —
  симметрично приглашению: `summarizeResponse` (чистое ядро: ответ+версия → строки «вопрос → значение»
  по стабильному `question_key`, НЕ агрегат — один респондент) → `buildSurveyResultActivity`
  (`completed:'Y'` — активность-ЗАПИСЬ; сводка по блоку на строку; кнопка «Открыть результат»).
  Постинг на submit + result-viewer (HTML/PDF) + smoke — #18.
...
- **Фаза D 🟠🟡** — **#17** авто-триггер `ONCRMDEALUPDATE` (эндпоинт `/api/b24/deal-update` + `event.bind`
  сделаны в коде; остаётся **дедуп/детекция перехода — блокер перед включением**, живой smoke, доставка ссылки,
  робот-эндпоинт `/api/b24/robot`); **#18** результат → таймлайн сделки + просмотр/PDF (**ядро ✅** — свод
  ответа + билдер активности; постинг/viewer/smoke впереди); рейтинг приложения + виджет
  обратной связи (UI по b24ui).
- **Фаза E 🟡** — **#4** общий стор анти-абьюза (Redis/PG) + `X-Forwarded-For` для мульти-инстанса
  (durable invitation-стор ОБЯЗАН персистить per-invitation `expiresAt` — иначе `linkTtlSeconds` сбросится
  на дефолт после рестарта / на другом инстансе); AI-разбор ответов (`answer_insight`).
- **Фаза F ⏳ (продукт/связка — из уточнения процесса 2026-07-24, `docs/process.md`)** — доставка приглашения
  через **настраиваемое дело** (`crm.activity.configurable.add`) + кнопка «Отправить приглашение» (виджет
  `CRM_DEAL_DETAIL_ACTIVITY`: `setLayout`/`bindLayoutEventCallback`) — **ядро билдера ✅** (`activity.ts`,
  сверено с live-verified соседом; постинг/виджет/smoke — впереди); **триггер завершения** → результат в
  таймлайн (#18 — **ядро ✅**: `summarizeResponse` + `buildSurveyResultActivity`; постинг/viewer/smoke впереди);
  **срок доступности ссылки** на каждый опрос (5 мин–5 дней, `invitationPolicy.linkTtlSeconds`
  — **ядро ✅**: поле схемы + проводка в TTL приглашения на выписке; остаётся поле в UI-конструкторе);
