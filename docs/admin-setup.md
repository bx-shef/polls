...
| Установка → «интеграция не сконфигурирована» (`503`) | нет `NUXT_B24_CLIENT_ID/SECRET`, БД или ключа шифрования | задать OAuth-креды приложения + `DATABASE_URL` + `NUXT_BITRIX_TOKEN_KEY` (fail-closed) |
| Установка → «слишком много попыток установки» (`429`) | сработал rate-limit install (20/60с; за nginx — общий bucket на инстанс) | флуд/повторные попытки — подождать ~минуту. In-memory на инстанс, общий стор — #4. В логах — `b24_install_ratelimited` |
| Установка → «недопустимый домен портала» (`400`) | домен гранта/POST не прошёл allowlist `*.bitrix24.<tld>` (SSRF-гард перед исходящим REST) | легитимный ОБЛАЧНЫЙ портал не должен это ловить; self-hosted/box (кастомный домен) сейчас не поддержан. В логах — `b24_install_bad_domain` |
