...
/**
 * ⚠ Здесь только то, что функция РЕАЛЬНО читает.
 *
 * Сначала сюда приезжали ещё портал, сущность, код и версия опроса — вызывающий старательно
 * их собирал, а `createInvitation` не смотрела ни на одно. Форма входа обещала «одну доменную
 * операцию» из `docs/PROCESS.md`, раздел 6, а делала минт токена со сроком. Панель ревью PR #18
 * нашла это дважды, с двух сторон. Остальные поля вернутся вместе с роботом — тогда же,
 * когда появится дедуп-ключ, которому они и нужны.
 */
