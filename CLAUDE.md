...
pnpm lint         # ESLint, типовые правила: забытый await (#165). tsc ловит его ТОЛЬКО там, где
                  # результат разыменовывают; `if (!promise)` и отброшенный промис он пропускает.
                  # Требует `pnpm nuxt:prepare` (правила по server/ и app/ читают .nuxt/tsconfig.*)
...
`webServer` поднимает собранное приложение, `pnpm test:visual` снимает реальный SSR-рендер (60 эталонов).
