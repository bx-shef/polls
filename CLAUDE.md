...
NPS/CSAT по группам через единый хелпер `breakdown` (имена денормализованы в контексте:
`companyName`/`dealCategoryName`/`responsibleName`/`productName`, фолбэк на ID; группа с выборкой < порога
И узкая метрика подавлены — анонимность среза); карточки рендерит общий `BreakdownCard.vue`. Фильтр по версии (`?version=N`, `byVersion`) — весь
