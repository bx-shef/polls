...
>
> ⚠️ **Статус источника (2026-06-19):** живой референс-прототип больше недоступен
> (см. шапку [`brief.md`](./brief.md)). Токены/раскладки/айдентика ниже — замороженный
> канон по конспекту, перепроверить против оригинала нельзя; визуальную сверку ведём
> против этих доков и собственного рендера (гейт скриншотов — issue #13).
>
...
- **Логика прохождения — готова в ядре.** Навигация, `exclusive`, «Другое», валидация
  шага, persist и маппинг в `Submission` реализованы в `src/client/survey-fill.ts`
  (`SurveyFill`, под тестами). Vue-композабл `useSurvey` лишь оборачивает класс
  реактивностью (`shallowRef`/`computed`) — не дублирует логику.
