...
## Маппинг: прочие сущности (лид / смарт-процесс / контакт / компания)

Фаза мульти-сущность (`src/bitrix24/entity-event.ts`) обобщает датчик на не-сделки. `CrmContext`
тот же; `dealStageId` несёт **обобщённый триггер-ключ** (имя историческое, см. `glossary.md`):

| `CrmContext`     | Лид (`crm.lead.get`) | Смарт-процесс (`crm.item.get`) | Контакт (`crm.contact.get`) | Компания (`crm.company.get`) |
| ---------------- | -------------------- | ------------------------------ | --------------------------- | ---------------------------- |
| `dealStageId`    | `STATUS_ID`          | `stageId`                      | — (нет стадии)              | — (нет стадии)               |
| `companyId`      | `COMPANY_ID`         | `companyId`                    | `COMPANY_ID`                | `ID` (сама компания)         |
| `contactId`      | `CONTACT_ID`         | `contactId`                    | `ID` (сам контакт)          | —                            |
| `responsibleId`  | `ASSIGNED_BY_ID`     | `assignedById`                 | `ASSIGNED_BY_ID`            | `ASSIGNED_BY_ID`             |
| `dealAmount`     | `OPPORTUNITY` (бюджет лида) | `opportunity`            | —                           | —                            |

Поля смарт-процесса — в нижнем camelCase (динамические типы). У контакта/компании пайплайн-стадии
нет → их опросы запускаются **вручную из виджета карточки**, не по стадии. `dealAmount` лида — это
«бюджет», у СП — `opportunity`: семантически не равны сумме сделки, но кладутся в одно поле (учесть в
KPI при необходимости). **Smoke на живом портале для лида/СП/контакта/компании НЕ проводился** —
маппинг из документации REST; проверить при появлении соответствующих binding'ов (#17).

**Диспетчер (#34):** `entityToCrmContext(entityType, item)` (`entity-event.ts`) роутит `item` на
`ENTITY_MAPPERS` (включая `deal → dealToCrmContext`); задача не поддержана этим путём (свой ручной виджет).
`entityGet(client, entityType, id, spaEntityTypeId?)` (`client.ts`) — REST-загрузчик: `crm.<entity>.get`
для deal/lead/contact/company; `crm.item.get` для spa (нужен `spaEntityTypeId`, разворот `{ item }`,
пустой → ошибка). Вызов — **после `verifyApplicationToken`** (анти-форджери/IDOR); контекст →
`handleDealTrigger` (`trigger.ts`) по стадии. Эндпоинт/`event.bind` — фаза связки (#34).

## Задача (`TASK_VIEW_SIDEBAR`) — ручной запуск

У задачи нет стадии воронки → автотриггер по стадии неприменим; опрос запускается **вручную из
карточки задачи** (виджет `app/pages/b24/task-widget.vue` + эндпоинт `server/api/b24/task-invite.post.ts`,
зеркало deal-invite). `tasks.task.get` → `taskToCrmContext` (`src/bitrix24/task.ts`):

| `CrmContext`    | Поле задачи |
| --------------- | ----------- |
| `responsibleId` | `responsibleId`/`RESPONSIBLE_ID` (v2) или `responsible.id` (v3) |
| `responsibleName` | `responsible.name` (v3; PII, редакция #31) |
| `dealId`/`contactId`/`companyId` | CRM-привязки `crmItemIds` (v3) / `ufCrmTask` (v2), формат `<PREFIX>_<id>`: `D`=сделка, `C`=контакт, `CO`=компания; лид (`L`) игнорируется |

`select` при `tasks.task.get` ОБЯЗАТЕЛЕН (`['*','UF_CRM_TASK']`) — привязки/UF по умолчанию не возвращаются.
Точный код плейсмента и набор полей сверить на портале (`placement.list`/живой smoke). Автотриггер по
статусу задачи (`ONTASKUPDATE`) — не реализован (см. `docs/issues.md`).
...
6. **`products` нужно обогащать ОТДЕЛЬНЫМ вызовом.** `crm.deal.get` НЕ возвращает
   товарные позиции — их даёт `crm.deal.productrows.get` (`PRODUCT_ID`/`PRODUCT_NAME`).
   Прод-путь (`dealToCrmContext` + `deal-invite`) раньше их не тянул → срез дашборда
   «услуга/товар» был бы пуст на реальных данных (в dev маскировалось seed'ом). Исправлено:
   `dealToCrmContext(deal, productRows?)` + `dealProductRows` (best-effort, провал логируется
   `b24_deal_productrows_fail`). `mapProductRows` **усекает** до капов схемы (≤50 позиций,
   имя ≤500) — иначе крупная сделка уронила бы `parse` → 502; и **дедупит** по `productId`
   (одна позиция несколькими строками — цена/скидка — не задваивает `byProduct`).
   **Известное ограничение:** free-form-строки товарной таблицы (`PRODUCT_ID=0`, услуга без
   привязки к каталогу) отбрасываются — группировать по каталогу нечем; для сервис-сделок
   срез «услуга/товар» может остаться пустым (группировка по `productName` — follow-up).
   Товары для lead/смарт-процесса — follow-up (те же `crm.<entity>.productrows.get`).

> **Живая сверка 2026-07-23** (тест-портал `*.bitrix24.com`, inbound-вебхук `B24_WEBHOOK_URL`): read-путь
> deal → `CrmContext` → 4-уровневая агрегация прошёл (8/8 сделок валидны по схеме); резолв
> компании/товарных позиций ✓. **Вокабуляр стадий подтверждён:** дефолтная воронка — `NEW`,
> `PREPARATION`, `PREPAYMENT_INVOICE`, `EXECUTING`, `FINAL_INVOICE`, `WON`, `LOSE`, `APOLOGY`;
> кастомная воронка (`C1`) — те же с префиксом `C1:` (`C1:WON`…). Это и есть формат `triggerStages`.
> `dealToCrmContext(deal, productRows)` вживую отдал `products:[{13,"[TEST] Внедрение"}]` для `C1:NEW`-сделки.
...
- **[#17](https://github.com/bx-shef/polls/issues/17) (роботы автоматизации — предпочтительный триггер)** —
  ...
  **Регистрация (при установке, через наш `callMethod`-хелпер поверх `actions.v2`):** `bizproc.robot.add` с `CODE` (уникальный),
