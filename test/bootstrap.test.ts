...
describe('bootstrap прод-стора (#6, pglite)', () => {
  ...
  describe('isPlaceholderPortal — можно ли сеять демо (#47/#49)', () => {
    it('плейсхолдер → да; настоящий портал → нет', async () => {
      // ⚠️ Смысл гейта: с фолбэком «всегда плейсхолдер» он при штатном старте истинен всегда, но
      // остаётся страховкой от регресса выбора фолбэка — окажись порталом по умолчанию боевой
      // тенант, boot засеял бы ему демо-опрос и дюжину выдуманных ответов, и снаружи это выглядело
      // бы нормальной работой.
