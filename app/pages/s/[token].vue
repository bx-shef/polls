...
useHead({
  ...
  // ⚠ Схема объявляется НА КОРНЕ и только на этой странице. Остальное приложение светлое
  // принудительно (`app/app.vue`), а лист анкеты тёмный по макету — и без этой строки
  // респонденту достаётся светлая полоса прокрутки вдоль тёмной страницы: полосу красит
  // браузер по схеме КОРНЯ, до которой `color-scheme` на `.page` не дотягивается.
  // Заодно снимается зависимость от системной темы: раньше полоса была тёмной только
  // у того, у кого тёмная система.
  htmlAttrs: { style: 'color-scheme: dark' },
  // ⚠ И фон холста — тоже отсюда. Всё приложение с 24.09 светлое принудительно, значит
  // `body` белый, а бланк тёмный: при резиновой прокрутке на телефоне над ним вспыхивала бы
  // белая полоса. До `body` не дотягивается `scoped`-стиль страницы, а НЕ-scoped блок
  // тут ловушка: он остаётся загруженным и после ухода со страницы, а очевидное условие
  // `body:has(.page)` красит заодно ЛЕНДИНГ — у него корень тоже `<main class="page">`.
  // `useHead` снимает вопрос по построению: атрибут живёт ровно пока открыт этот маршрут.
  //
  // Цвет повторяет `--sheet` литералом: переменная объявлена НА `.page`, снаружи её не видно.
  bodyAttrs: { style: 'background: #2c2c36' },
})
...
/*
  ...
  ⚠ СТРАНИЦА ТЁМНАЯ ВСЕГДА, а не следует системной теме. Остальное приложение с 24.09
  светлое принудительно — тоже всегда, — и эта страница единственное исключение из правила,
  а не остаток прежнего «обе темы». Причина в том, что она не наш экран: респондент
  открывает её один раз на минуту по ссылке из письма, и это фирменный бланк заказчика.
  Светлый вариант того же макета — второй дизайн, которого никто не рисовал, а не «та же
  страница посветлее».
...
*/
.page {
  /* Схема нужна и здесь: `htmlAttrs` в `useHead` красит полосу прокрутки документа,
     а это — всё, что рисует браузер ВНУТРИ листа: каретка, выделение, поле ввода. */
  color-scheme: dark;
