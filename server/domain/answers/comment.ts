 ...
 *
 * ⚠ УГЛОВЫЕ СКОБКИ ТОЖЕ, и это решение теперь стоит на замере, а не на опасении. Заведено
 * панелью ревью PR #37–#39 из страха, что портал подставит HTML (`3`). Замерено: по умолчанию
 * он ставит `2`, а лента при двойке рисует `<b>` и `<a href>` БУКВАЛЬНО, тегами — то есть
 * оживить HTML из ответа постороннего человека сегодня нечем.
 *
 * Обезвреживание тем не менее оставлено: замер сделан при типе `2`, а опасение было про `3`,
 * и её не пробовали. Цена несимметрична — `＜` вместо `<` в ответе вроде «хотелось бы срок
 * < 3 дней» портит текст заметно, но безобидно; живой `<img src=x onerror=…>` в чужой CRM
 * не безобиден и навсегда. Вся тяжесть решения лежит на двойке в `DESCRIPTION_TYPE`, и она
 * закреплена гвардом.
 *
