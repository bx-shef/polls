  ...
  return {
    ...
    async submit({ ip, body }: SubmitInput): Promise<ApiResult> {
      ...
      try {
        ...
        /**
         * CRM-снимок из приглашения (#3). Расходуем ПОСЛЕ валидации ответов — чтобы 422 не сжигал
         * неповторимое приглашение (в отличие от nonce, который переиздаётся через /api/session).
         * Нет токена → context пуст (back-compat).
         *
         * ⚠️ **Пара «расход + запись» НЕ атомарна, и с durable-стором приглашений (#4) цена этого
         * выросла.** Раньше отказ записи после расхода лечился рестартом — стор был в памяти.
         * Теперь нет: ссылка мертва навсегда, ответа нет, человеку говорят «вы уже прошли опрос», а
         * переиздать ссылку может только менеджер. Окно узкое (оба стора — одна и та же БД, и её
         * отказ обычно роняет уже `consume`), но оно есть, и это единственный новый способ потерять
         * ответ клиента.
         *
         * ⚠️ **Очевидная перестановка «прочитать → записать → погасить» это НЕ чинит — проверено
         * исполнением.** Она ломает более важное свойство: двое по одной ссылке получают тогда оба
         * 200, потому что второй ответ молча съедается дедупом `uq_response_invitation_token`, и
         * человек, чьи ответы выброшены, об этом не узнаёт. Сегодняшний порядок отвечает ему честным
         * 409 («ссылка уже использована»). Молчаливый приём хуже редкой потери — и ровно с таким
         * классом дефектов борется вся эта работа.
         *
         * Настоящее лечение требует, чтобы `addResponse` СООБЩАЛ, записал он или отбросил по дедупу:
         * тогда порядок «прочитать → записать → погасить» даёт и честный 409, и живую ссылку при
         * отказе. Это правка порта `IStore`, и делается она отдельно — issue #170.
         */
