...
> - **SSRF-allowlist для `domain`/`clientEndpoint`.** Когда грант НЕ вернул ни `domain`, ни
>   `client_endpoint`, `applyVerifiedTokens` фолбэчит на клиент-присланные значения. Владелец портала
>   мог бы подставить внутренний URL как endpoint «своего» портала → SSRF при исходящих REST. Добавить
>   валидацию хоста по allowlist `*.bitrix24.<tld>` (переиспользовать `isAllowedPortalDomain`).
...
> - **rate-limit на `/api/b24/install`** (#4) — эндпоинт без лимитера; после §2.3 он стал **исходящим
>   амплификатором** (аноним → наш сервер → рефреш на `oauth.bitrix.info`) → приоритет follow-up выше.
