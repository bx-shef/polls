...
export const linkIndex = pgTable('link_index', {
  ...
  /**
   * Шапка анкеты на момент выпуска: компания, проект, кого спрашиваем, кто спрашивает.
   *
   * ⚠ ЕДИНСТВЕННОЕ МЕСТО В СХЕМЕ, ГДЕ ЛЕЖАТ ИМЕНА ЛЮДЕЙ. Заведено решением владельца 23.09,
   * и вопрос «почему не в смарт-процессе» имеет ответ: публичная страница анкеты не ходит
   * в портал по инварианту, значит показать ей компанию и контакт можно только тем, что мы
   * сохранили сами. Та же причина, по которой рядом лежит кэш схем версий.
   *
   * ⚠ Снимок, а не ссылка. Сделку переименуют, контакт заменят — а человек отвечал
   * на анкету, в шапке которой стояло вот это. Задним числом шапка не меняется, как
   * не меняется и сама версия анкеты.
   *
