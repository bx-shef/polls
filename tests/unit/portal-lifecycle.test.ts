...
describe('распознавание мёртвого гранта', () => {
  it('узнаёт отказ сервера авторизации, после которого грант мёртв', () => {
    // Описание — дословно из документации «Коды ошибок сервера авторизации».
    expect(isDeadGrant(refusal('invalid_grant', 'Переданы некорректные авторизационные данные'))).toBe(true)
  })

  it('НЕ считает мёртвым `invalid_client`', () => {
    // ⚠ Документация про него: «Переданы некорректные данные клиента… Проверьте `client_id`
    // и `client_secret`» — то есть про НАШУ пару, одну на весь флот. Незаконченная ротация
    // секрета пометила бы мёртвыми все порталы разом. Стоял в списке, убран повторной
    // панелью ревью PR #34: под собственное правило («только то, что не может быть вызвано
    // нашей ошибкой») он не подходит точнее всех прочих исключённых.
    expect(isDeadGrant(refusal('invalid_client', 'Переданы некорректные данные клиента'))).toBe(false)
  })
