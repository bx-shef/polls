...
> ДО `handleInstall` (ротация → `verifiedAuth` во все пути). **Идемпотентность двойной доставки**:
> Bitrix шлёт install-страницу И `ONAPPINSTALL` на тот же URL; второй запрос рефрешит уже
> ротированный первым refresh_token → `invalid_grant`; если портал уже установлен — отдаём
> `FINISH_HTML` (гонка, не подделка), но **только** для `refresh_rejected_*`, не для `member_mismatch`.
...
> - **rate-limit на `/api/b24/install`** (#4) — сейчас install-эндпоинт без лимитера; злоупотребление
>   рефрешами гасится 503-путём, но проактивный лимит желателен.
