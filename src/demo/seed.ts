...
export function draftV1(): SurveyDraft {
  return {
    ...
    // ⚠️ Демо обещает на интро «Анонимно» — значит индивидуальный результат в карточку сделки НЕ
    // уходит (#18). Обещание респонденту и настройка обязаны совпадать, и здесь это не педантизм:
    // ключ демо совпадает с `DEFAULT_SURVEY_KEY`, демо засевается в пустую базу, а установка
    // присваивает плейсхолдер вместе с ним — на портале без заданных `SURVEY_KEY_*` виджет выпишет
    // приглашение именно на этот опрос, и живой клиент прочитал бы «Анонимно» перед тем, как его
    // ответы легли бы в карточку. Развязка демо и продуктового ключа — #187.
    // Триггерных стадий у демо нет: событием оно не запускается.
    invitationPolicy: { entityType: 'deal', triggerStages: [], channelOrder: ['email', 'sms'], resultToTimeline: false },
    questions: [
