...
export const linkIndex = pgTable('link_index', {
   ...
   * сюда. Первый обязан быть первым, и цена ошибки несимметрична: публичная страница анкеты
   * не ходит в портал ПО ИНВАРИАНТУ, поэтому строка здесь без строки в `survey_templates` —
   * это пожизненный 503 по уже отправленной человеку ссылке. Отозвать её нельзя, она у него
   * в письме.
   *
   * До этого ключа порядок держался комментарием в шапке файла и вниманием того, кто файл
   * правит. Теперь вставить ссылку раньше схемы просто не получится.
   *
   * ⚠ Удаление обратной стороной: пока на схему ссылается хоть одна ссылка, строку кэша
   * не удалить. Чистки кэша у нас нет вовсе, но когда появится — ей придётся считаться
   * с ключом, и это правильно: выбросить схему живой ссылки значит сломать её страницу.
   * Решение владельца записано в `docs/PROCESS.md`, а не получено побочным эффектом миграции.
   */
  foreignKey({
    columns: [table.portalId, table.surveyCode, table.surveyVersion],
