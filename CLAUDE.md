...
`session.get`, `submit.post`, `survey/[key]/current.get`, `health.get`. Логика — в ядре,
обёртки только мапят `event → api.*(...)`/статус. Типизируются Nitro-tsconfig (не ядровым
`pnpm check`); живой smoke — `pnpm build` + curl (см. PR Nitro-привязки).
