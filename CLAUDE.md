...
Фронта в ядре ещё нет — правило вступает в силу с Nuxt/b24ui-слоя. **ВАЖНО:**
правка UI/CSS/вёрстки не готова, пока результат не увиден глазами: отрендерить →
скриншот → сравнить с макетом → перечислить отличия → починить. Минимум: брейкпоинты
(375px/десктоп), состояния (пусто/ошибка/длинный текст), hover/focus/disabled,
светлая/тёмная тема. **Приватность:** скриншоты живого портала клиента (данные CRM,
домены, токены) НЕ коммитим, НЕ шлём в облачный чат, НЕ кладём в CI — только
dev/staging с мок-данными или обезличенное. Автоскриншот закрепим `Stop`-хуком +
Playwright, когда появится фронт.
