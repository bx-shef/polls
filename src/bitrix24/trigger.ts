 ...
 *
 * Развязка нужна потому, что доставка — это REST, а ядро про REST ничего не знает и знать не должно.
 * Сама работа у ОБОИХ путей триггера одна и та же (#175): спросить портал, не висит ли уже дело по
 * этому переходу, и только потом создать — приглашение вместе с делом в таймлайне
 * (`invite-delivery.ts`). Различаются пути лишь тем, откуда берётся ключ перехода: у события — `ID`
 * записи истории стадий, у робота — момент срабатывания (`robotTransition`).
 *
...
export async function handleDealTrigger(deps: {
  ...
  /**
   * Как выписывать приглашение. Оба боевых пути триггера — событие и робот — передают сюда доставку
   * делом в таймлайне (#126 + #138 + #175).
   *
   * ⚠️ **Не задана** → фолбэк `issueWithoutDedup`: токен в базе, дела нет, ссылка не уходит никому.
   * Это НЕ «поведение по умолчанию», а путь без единой проверки — см. разбор у самой функции. Живой
   * вызывающий, не передающий `issue`, и есть дефект #175 в его исходном виде.
   */
...
/**
 * Выписка БЕЗ дедупа и БЕЗ доставки — фолбэк, когда `issue` не задан. Боевых вызывающих у него
 * больше НЕТ: и событие, и робот передают доставку (#175). Остаются ядровые тесты.
 *
 ...
 * ни защиты от грозди. Робот прожил так до #175 — приглашение в базе, ссылка никуда не уходит.
 * Новый вход (лид, смарт-процесс) молча унаследовал бы то же самое, и тесты остались бы зелёными —
 * поэтому выбор должен быть виден на месте вызова, а структурный гард перечисляет триггер-роуты
 * поимённо (`test/tenant-routes.test.ts`).
 */
