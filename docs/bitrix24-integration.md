...
| `dealCategoryName`  | `crm.category.get`/`crm.dealcategory.get` → `NAME` | направление (подпись среза) |
...
к CRM-справочникам. Опциональны: при отсутствии срез падает на ID (`#11`). Боевой адаптер
резолвит их одним батчем доп-запросов к CRM при снятии контекста.
