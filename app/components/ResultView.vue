...
/**
 * Дата ответа. Локаль прибита русской — приложение русскоязычное целиком; языка портала мы не
 * спрашиваем, и обещать это в комментарии не надо.
 *
 * ⚠️ Битую строку показываем КАК ЕСТЬ, а не прячем: «—» вместо даты читается как отсутствие данных,
 * хотя данные есть и проблема в формате.
 */
...
<template>
  <article class="result">
    ...
    <!-- ⚠️ «Мы не показали» и «клиент не ответил» — разные вещи, и молчать о первом нельзя. -->
    <p v-if="view.truncated > 0" class="mt-1 text-sm text-gray-600 dark:text-gray-300">
      Показаны не все ответы: ещё {{ view.truncated }} не поместились.
    </p>
...
@media print {
  ...
  /*
   * ⚠️ Цвет дожимается на ПОТОМКАХ, а не только на контейнере. У меток вопросов и подписей стоят
   * утилиты `dark:text-gray-300`, и их специфичность выше: у сотрудника с тёмной темой ОС на бумагу
   * уходило светло-серое по белому (≈1,4:1) — то есть печать теряла ровно левую половину пары
   * «вопрос → ответ», ради которой её и делают. Фоны браузер по умолчанию не печатает, поэтому
   * «тёмная тема на бумаге» не спасает.
   */
  .result,
  .result * {
    color: #000 !important;
