...
describe.skipIf(!enabled)('жизнь портала после установки', () => {
  ...
  it('не стирает, когда под отсчётом больше трети флота', async () => {
    ...
    // портале навсегда.
    //
    // ⚠ ДОЛЯ ВЗЯТА С ЗАПАСОМ (6 из 13 — почти половина), а не впритык к порогу. Впритык
    // здесь стояло: 4 помеченных из 11, то есть 36 % против порога в 33 %. Предохранитель
    // считает по ВСЕЙ таблице, а не по тому, что насеял тест, — и одна посторонняя строка
    // в базе делала долю ровно третью, порог не срабатывал, и набор падал. Поймано живьём:
    // `pnpm check` против базы, где лежал один служебный портал. Сам тест при этом не врёт
    // в опасную сторону — он краснеет, а не зеленеет, — но красный от чужой строки тест
    // перестают читать.
    for (let i = 0; i < 6; i += 1) await seedPortal(new Date(NOW.getTime() - (OVERDUE + i) * DAY_MS))
