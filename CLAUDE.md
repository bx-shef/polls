...
```bash
pnpm check        # всё разом: typecheck(ядро) + граница ~core + lint + typecheck:app + typecheck:server + test (покрытие) + verify
...
- Подавление малых выборок — **два уровня, и живут они в разных местах**: (1) гейт по общему N
  (`ANONYMITY_THRESHOLD = 5` / `meetsAnonymity`) — ответственность слоя чтения/PgStore; (2)
  k-анонимность **по ячейкам** распределения (`suppressSmallBins`, `src/domain/aggregate.ts`) — чистый
  хелпер, который обязан позвать КАЖДЫЙ потребитель распределения: сейчас это
  `server/utils/dashboard-view.ts`, а `distributionFor` и `PgStore.aggregateDistribution` отдают ячейки
  СЫРЫМИ (сырое распределение нужно и для расчётов). ⚠️ Дашборд уже переведён на SQL-агрегацию (#49)
  через порт `IStore.dashboardAggregates` — подавление ГРУПП живёт внутри порта, ячеек — снаружи, и
  это единственное исключение. Новый потребитель распределения обязан позвать `suppressSmallBins`
  сам: забудет — защита исчезнет без единого красного теста. Пагинация и tenant-изоляция (`portalId`) — слой
  чтения/PgStore, не «сырые» агрегаты.
...
- **Гейт мержа:** `pnpm check` зелёный (typecheck ядра + граница `~core` + `lint` + `typecheck:app`/`:server` +
...
- **Ядро (`src/`):** `pnpm typecheck` + `pnpm lint` + тесты с покрытием ≥85% (по факту ~100%) + `pnpm verify`
...
`webServer` поднимает собранное приложение, `pnpm test:visual` снимает реальный SSR-рендер (66 эталонов).
...
GitHub Actions: `ci.yml` (typecheck ядра + граница `~core` + `lint` + `typecheck:app` vue-tsc + тесты
с покрытием + verify), `docker-build.yml` (сборка прод-образа на PR — **без публикации**, гейт от поломки образа до
