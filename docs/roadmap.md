...
[✅ ядро] → [✅ SurveyFill (мозг контура A)] → [✅ виз. гейт #13] → [✅ Nuxt+b24ui каркас]
              → [✅ Nitro-привязка createApi] → [🔶 экраны опроса (контур A): happy-path готов]
...
| 3 | Экраны опроса (контур A): Интро/Опрос/Спасибо (`/s/:key`) + `useSurvey` | 0, 0.5, 1, 2 | 🔶 happy-path + гейт intro/survey/thanks/error ×(light+dark) + persist/deep-link/тёмная тема ✅; загрузка/пусто → #34 |
