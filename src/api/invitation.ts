 ...
 * ⚠️ Но **до процедуры удаления данных портала приглашения всё ещё не доходят** — и не по вине
 * скоупа: рантайм пишет под портал `__local__` (`ensureDefaultPortal`), а `deletePortal(memberId)`
 * чистит строки портала с настоящим `member_id`. Тот же разрыв давно есть у `response`; issue #171.
 *
