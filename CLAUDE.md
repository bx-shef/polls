...
pnpm typecheck:app   # vue-tsc-typecheck app/+server/ (Nuxt; отд. шаг CI, #36). Перед — pnpm nuxt:prepare
...
- `bitrix24/crypto.ts` (`TokenCipher` AES-256-GCM с `kid` в blob — форвард-совместимость
  ...
  `expires`, authoritative `domain`/`memberId` из гранта, `clientEndpoint` ВСЕГДА = `https://<domain>/rest/` (присланный/
  грант-endpoint как host не используются → SSRF)), вызывается в install-хендлере ДО `handleInstall` (синхронный рефреш
  обёрнут `AbortSignal.timeout` 10с; `domain` валидируется `isAllowedPortalDomain` ДО REST на домен портала → 400;
  rate-limit `allowB24Install` 20/60с ДО рефреша — анти-амплификация; регекс `isAllowedPortalDomain` ужат от «двойной TLD»-дыры SSRF);
...
`postinstall: nuxt prepare`; CI-typecheck `app/` — отдельным шагом `pnpm typecheck:app` (vue-tsc);
[#36](https://github.com/bx-shef/polls/issues/36) — закрыт).
...
чистая логика разбора — под юнит-тестами (`test/core-boundary.test.ts`). Типы `app/`+`server/` гейтит
`pnpm typecheck:app` (vue-tsc, отдельный шаг CI — #36 закрыт): ядровой `pnpm typecheck` остаётся независимым.
...
`GET /api/survey/:key/current` (+ HTTP-кэш #30: ETag `sv-<key>-<versionNo>-s<schemaVersion>` + `If-None-Match`→304, `no-cache`; чистые `versionETag`/`etagMatches`/`cacheDecision` в `src/api/http-cache.ts`, под тестами), `GET /api/health`, `GET /api/dashboard/:key` (контур B),
...
Nitro-tsconfig, не ядровым `pnpm check` (CI-typecheck server/app — `pnpm typecheck:app`, #36 закрыт); живой smoke — `pnpm build` + curl.
...
- **read-API / PgStore** — сделаны: CRUD + tenant-изоляция, keyset-пагинация,
  ...
  [#31](https://github.com/bx-shef/polls/issues/31)); ETag/условный-GET для `survey/:key/current`
  сделан ([#30](https://github.com/bx-shef/polls/issues/30)), остаётся store-side TTL-кэш `currentVersion`.
