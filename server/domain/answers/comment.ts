 ...
 * по умолчанию подставит HTML (`3`), — и опасение проверено живьём 24.09 по issue #26:
 * **по умолчанию портал ставит `2` (BB-код)**, а не HTML. То есть окна, в котором угловые
 * скобки респондента ожили бы разметкой, у нас нет.
 *
 * Обезвреживание тем не менее остаётся, и по двум причинам, а не по инерции. Первая: тип
 * описания едет ОТДЕЛЬНЫМ вызовом, и он же — единственное место, где тройка может появиться;
 * защита, снятая «потому что сейчас её некому пробить», возвращается дороже. Вторая: цена
 * защиты — четыре символа начертания в тексте, который их почти никогда не содержит, а цена
 * ошибки — `<img src=x onerror=…>` от постороннего человека, живущий в чужой CRM навсегда.
 *
 * Цена — четыре символа начертания в тексте, который их почти никогда не содержит.
 */
