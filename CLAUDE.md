...
`GET /api/survey/:key/current` (+ HTTP-кэш #30: ETag `(key@versionNo)` + `If-None-Match`→304, `no-cache`; чистые `versionETag`/`etagMatches` в `src/api/http-cache.ts`), `GET /api/health`, `GET /api/dashboard/:key` (контур B),
