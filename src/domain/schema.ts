...
export const invitationPolicySchema = z.object({
   ...
   * автор опроса, и в конструкторе они встанут рядом именно поэтому. Демо-опрос (`demo/seed.ts`)
   * чип несёт, а политики не имеет вовсе — он и служит фикстурой пути «политики нет», а портала у
   * него не бывает, так что записывать всё равно некуда.
   */
