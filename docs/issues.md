...
| #18 | Результат анкеты → таймлайн сделки (`crm.activity.*`) + result-viewer (HTML, печать/PDF) | bitrix24 | **ядро ✅** (`summarizeResponse` + `buildSurveyResultActivity` — сводка ответа → активность `completed:'Y'`, под тестами) | остаётся: постинг на submit + result-viewer (HTML/PDF) + живой smoke; симметрия к #17; зависит от OAuth (#3 ✅), binding-слоя (#17), PII (#10/#31) |
