...
export interface ApiDeps {
   ...
   *
   * ⚠️ **Ответ клиента дороже отметки в таймлайне.** Хук вызывается ПОСЛЕ записи, его отказ на
   * ответ 200 не влияет и наружу не пробрасывается: заставить человека заполнять анкету заново из-за
   * недоступности портала — худшее, что тут можно сделать. Ядро само ошибку не глотает (иначе
   * молчание было бы неотличимо от успеха) — глотает и логирует вызывающий, у него есть логгер.
   */
...
export interface AnsweredInfo {
  ...
  context: CrmContext
  /** Момент записи (серверные часы) — им же помечается закрытие дела. */
  at: Date
}
