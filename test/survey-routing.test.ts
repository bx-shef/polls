...
describe('surveyKeyForEntity', () => {
  it('берёт ключ из routing для заданной сущности', () => {
    expect(surveyKeyForEntity('lead', { lead: 'lead_csat', deal: 'deal_nps' })).toBe('lead_csat')
    expect(surveyKeyForEntity('deal', { lead: 'lead_csat', deal: 'deal_nps' })).toBe('deal_nps')
  })
  it('падает на дефолт, если сущности нет в routing', () => {
    expect(surveyKeyForEntity('lead', { deal: 'deal_nps' })).toBe(DEFAULT_SURVEY_KEY)
    expect(surveyKeyForEntity('deal')).toBe(DEFAULT_SURVEY_KEY)
  })
  it('пустое/пробельное значение трактуется как незаданное', () => {
    expect(surveyKeyForEntity('lead', { lead: '   ' })).toBe(DEFAULT_SURVEY_KEY)
    expect(surveyKeyForEntity('lead', { lead: '' })).toBe(DEFAULT_SURVEY_KEY)
  })
  ...
  it('обрезает пробелы у ключа', () => {
    expect(surveyKeyForEntity('lead', { lead: '  lead_csat  ' })).toBe('lead_csat')
  })
})
...
describe('surveyRoutingFromEnv', () => {
  it('собирает SURVEY_KEY_<ENTITY> для всех сущностей', () => {
    ...
    expect(routing).toEqual({ deal: 'd', lead: 'l', spa: 's', contact: 'c', company: 'co' })
  })
  it('неизвестная сущность (SURVEY_KEY_TASK — вне ENTITY_TYPES) не подхватывается', () => {
    expect(surveyRoutingFromEnv({ SURVEY_KEY_TASK: 't' }).routing).toEqual({})
  })
  ...
  it('end-to-end: env → routing → ключ для лида', () => {
    const { routing, fallback } = surveyRoutingFromEnv({ SURVEY_KEY_LEAD: 'lead_done' })
    expect(surveyKeyForEntity('lead', routing, fallback)).toBe('lead_done')
