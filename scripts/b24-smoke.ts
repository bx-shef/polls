 ...
 *
 * Скоуп: это смоук пути ЧТЕНИЯ через входящий вебхук (deal → CrmContext → агрегация).
 * Прогон РЕЖИМА ПРИЛОЖЕНИЯ (install → handshake фрейма → виджеты → дашборд) — отдельный
 * ручной плейбук в `docs/project-map.md` («Ручной прогон режима приложения Bitrix24»).
 */
import {
  crmContextSchema,
