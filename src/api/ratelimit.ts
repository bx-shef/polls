 ...
 *
 * ⚠️ Общий стор для этого лимитера — **не Postgres**, в отличие от приглашений и nonce. `allow()`
 * зовётся до разбора тела на публичных путях (включая вебхук событий), поэтому durable-версия на БД
 * означала бы round-trip на каждый запрос — то есть под флудом лимитер бил бы ровно по тому ресурсу,
 * который защищает. Субстрат — Redis или ничего; цена «ничего» невелика: рестарт сбрасывает окно
 * ≤60 секунд, это fail-open на одно окно. Решение и порядок работ — карта, §Ключевые решения (#4).
 */
