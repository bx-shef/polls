...
export function checkEnv(env: Record<string, string | undefined>, opts: EnvCheckOptions = {}): EnvReport {
  ...
  // #183: боевой контур приложения ОБЯЗАН знать, чей он. Без `B24_EXPECTED_MEMBER_ID` накопленные до
  // установки данные присвоит ПЕРВЫЙ установившийся портал — кто бы он ни был, — и вместе с ними
  // получит право их стереть (uninstall с очисткой). Это ошибка, а не предупреждение: цена — чужие
  // ПДн, и владелец узнал бы о ней только по факту чужой установки.
  const expectedMember = val('B24_EXPECTED_MEMBER_ID')
  if (hasId && hasSecret && !expectedMember) {
    errors.push({
      name: 'B24_EXPECTED_MEMBER_ID',
      message: 'не задан при настроенной связке с порталом — накопленные до установки данные присвоит ПЕРВЫЙ установившийся портал, кто бы он ни был, и сможет их стереть. Укажите member_id своего портала (виден в логе установки и в параметрах фрейма)'
    })
  } else if (expectedMember && isPlaceholder(expectedMember)) {
    // Заглушка хуже пустоты: гейт «работает» и отклоняет ВСЕ установки, включая свою, — симптом
    // «403 на собственном портале» вместо причины «в переменной osta заглушка из примера».
    errors.push({
      name: 'B24_EXPECTED_MEMBER_ID',
      message: 'содержит заглушку из примера — гейт установки отклонит ВСЕ порталы, включая ваш. Укажите настоящий member_id'
    })
  } else if (expectedMember && !/^[0-9a-f]{32}$/i.test(expectedMember)) {
    // Форма member_id у Bitrix24 — 32 hex-символа. Иное значение с высокой вероятностью опечатка;
    // предупреждение, а не ошибка: форму Bitrix не документирует, и жёсткий отказ на валидном, но
    // нетипичном значении заблокировал бы установку владельцу.
    warnings.push({
      name: 'B24_EXPECTED_MEMBER_ID',
      message: 'не похоже на member_id (у Bitrix24 это 32 шестнадцатеричных символа) — если значение с опечаткой, установка вашего портала получит 403. Сверьте с логом установки'
    })
  }

  // Режим порталов: значения ровно два. Опечатка ⇒ ошибка ЗДЕСЬ, потому что рантайм на незнакомом
  // значении молча падает в `single` — безопасно, но неотличимо от намеренного, и «включил Маркет»
  // выяснилось бы отказом первой же чужой установки.
  const portalMode = val('B24_PORTAL_MODE')
  if (portalMode && portalMode !== 'single' && portalMode !== 'multi') {
    errors.push({
      name: 'B24_PORTAL_MODE',
      message: `значение не распознано — допустимы только single (один портал, чужая установка отклоняется) и multi (контур Маркета). Рантайм молча считает такое значение за single`
    })
  }
