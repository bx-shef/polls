...
> ✅ **Сделано** (PR #111): ядро `parseUninstallEvent`/`decideUninstall` (`uninstall.ts`) +
> `parseBracketForm` (`bracket-form.ts`, гард prototype-pollution) + ветка `ONAPPUNINSTALL` в
> install-хендлере → `deletePortal` при `CLEAN=1`. `application_token` берётся из зашифрованного
> blob токенов (колонка 0004 избыточна). Формат сверен с офиц. докой. Осталось: **живой smoke**
> доставки события; зависит от захвата `application_token` при install (смежно #17).
...
> ✅ **Сделано**: `bitrix24/verify-install.ts` — `verifyInstallMember` (рефреш присланного
> refresh_token → сверка authoritative `member_id`; **403 только при 400 `invalid_grant`/401 или
> mismatch**, всё прочее — **429 rate-limit/5xx/сеть/пустой member_id — 503** транзиент, чтобы не
> ложно-отвергать легитимную установку) + `applyVerifiedTokens` (чистая сборка `InstallAuth` из
> **ротированного** гранта: свежие access/refresh, пересчёт `expiresIn`, **сброс stale абсолютного
> `expires`** event-формата, authoritative `domain` из гранта, `clientEndpoint` **всегда деривится** из
> `domain` (`https://<domain>/rest/`; присланный/грант-endpoint как host не используются)). Вызов в install-хендлере
> ДО `handleInstall` (ротация → `verifiedAuth` во все пути). Синхронный верификационный рефреш —
> исходящий вызов внутри install-запроса → обёрнут `AbortSignal.timeout` (10с; ядро делегирует
> таймаут «слою деплоя», здесь этот слой — сам хендлер). **Идемпотентность двойной доставки** —
> чистая `decideInstallDoubleDispatch(reason, portalExists)`: Bitrix шлёт install-страницу И
> `ONAPPINSTALL` на тот же URL; второй запрос рефрешит уже ротированный refresh_token →
> `invalid_grant`; если портал уже установлен → `FINISH_HTML` (гонка), иначе видимая ошибка (не
> маскируем мисконфиг `client_secret`); `member_mismatch` и транзиент → всегда ошибка.
> Тесты 100% (`test/verify-install.test.ts`).
>
> **Сознательно НЕ портирован** отдельный `verifyInstallToken` донора (доказательство контроля
> **домена** дешёвым `profile`-вызовом): у polls рефреш+сверка authoritative `member_id` уже доказывает
> контроль домена (грант принадлежит порталу с этим member_id), а authoritative `domain` берётся из
> ответа гранта — дублирующий `profile`-вызов не нужен.
>
> **✅ Follow-up-хардеринг сделан** (отдельный PR):
> - **SSRF-allowlist для `domain`/`clientEndpoint`.** `applyVerifiedTokens` больше НЕ доверяет
>   клиент-присланному `clientEndpoint` как host'у — `clientEndpoint` **всегда** = `https://<domain>/rest/`.
>   Сам `domain` install-хендлер валидирует по `isAllowedPortalDomain` ДО исходящих REST на домен портала
>   (`registerIntegrations`) → недопустимый домен = 400 `b24_install_bad_domain`. REST-host полностью покрыт.
> - **Фикс дыры в `isAllowedPortalDomain`** (MAJOR из ревью, затрагивал и handshake фрейма #47): открытая
>   «двойная TLD»-ветка регекса пропускала `foo.bitrix24.evil.com` (`evil.com` — регистрируемый атакующим
>   домен, `bitrix24` — его поддомен) → SSRF. TLD ужат до ОДНОГО dot-free лейбла + явный `com.br`.
> - **rate-limit на `/api/b24/install`.** `allowB24Install` (SlidingWindowLimiter 20/60с) ДО исходящего
>   рефреша — гасит амплификатор (аноним → наш сервер → `oauth.bitrix.info`); uninstall не лимитируем.
>
> **Осталось (follow-up, не в этом PR):**
> - **domain-poisoning закрыт лишь частично.** `applyVerifiedTokens` привязывает authoritative
>   `domain` из гранта, но два member_id с одним `domain` схему изоляции не ломают на уровне БД.
>   Полное закрытие — `UNIQUE(portal.domain)` + `resolveMemberIdByDomain … ORDER BY` детерминизм
>   (миграция, отдельный PR).
> - **Остаточный install-оракул** (осознанно принят, low-severity): ветка `finish` при существующем
>   портале раскрывает факт установки тому, кто знает `member_id` (128-бит, не перебираемый; 1 бит,
>   записи нет). Полное закрытие противоречит видимости мисконфига — оставлено.
> - **Явный список облачных TLD Bitrix в `isAllowedPortalDomain`.** Сейчас TLD = любой один лейбл
>   (`[a-z]{2,}`) → остаточно `bitrix24.<регистрируемый-gTLD>` (напр. `bitrix24.app`) прошёл бы. Полное
>   закрытие — whitelist реальных облачных TLD Bitrix (нужен авторитетный список; fail-closed при пропуске).
>   В том же follow-up закрыть `serverEndpoint` (недоверенный `server_endpoint` из event-install течёт в
>   `B24OAuthParams.serverEndpoint`, allowlist по `domain` его не покрывает; сейчас не эксплуатируется —
>   `installToTokens` его отбрасывает, при install рефреша нет — но стоит валидировать/дропнуть).
> - **Self-hosted/box-порталы** (кастомный домен) сейчас ловят 400 на domain-гарде — если понадобятся,
>   пробросить конфигурируемый allowlist (`isAllowedPortalDomain` уже принимает свой RegExp параметром).
> - **Общий стор лимитов** для мульти-инстанса + `X-Forwarded-For` за прокси (#4) — сейчас лимитер
>   in-memory на инстанс (за nginx IP собирается в один bucket — для анти-амплификации ОК).
> - **Живой smoke** установки на портале (рефреш свежевыданного токена; двойная доставка page+event).
...
- **В polls.** Чистая `verifyInstallMember` в `src/bitrix24/` (у polls уже есть `Bitrix24OAuth`
  обмен/refresh — DI-инъекция); вызвать в install-конвейере **до** `handleInstall`. Гейт на
  `NUXT_B24_CLIENT_ID/SECRET`. Отдельный `verifyInstallToken` (profile-проба домена) намеренно не
  портирован — см. блок «✅ Сделано» выше.
