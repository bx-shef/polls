 ...
 * мёртвым» обязан быть один: подмести приглашения ОДНОГО портала (`sweepExpired` — часть порта
 * `InvitationStore`; боевого вызывающего у него сегодня нет, его держат тесты tenant-изоляции) и
 * подмести ВСЕ (суточный крон удержания ПДн, #49 — единственный боевой вызывающий). Две копии
