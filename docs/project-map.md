...
- **`bitrix24/`** — интеграция (server-only):
  ...
  - `install.ts` — оркестрация установки (робот `bizproc.robot.add` + `event.bind ONCRMDEALUPDATE`
    (`surveyEventBindParams`) + плейсменты).
...
- **Установка** `/api/b24/install` (два формата: install-страница + событие `ONAPPINSTALL`):
  ...
  **503** транзиент; рефреш обёрнут `AbortSignal.timeout` 10с) → `applyVerifiedTokens` (ротированный
  ...
  `isAllowedPortalDomain` ДО REST → `handleInstall` (сохранить токены + робот `bizproc.robot.add` +
  `event.bind ONCRMDEALUPDATE` → `/api/b24/deal-update` (#17) + 2 плейсмента; каждая регистрация
  толерантна к ошибке). Rate-limit `allowB24Install` 20/60с ДО рефреша (анти-амплификация). Идемпотентность
