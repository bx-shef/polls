...
> `expires`** event-формата, authoritative `domain`/`clientEndpoint` из гранта). Вызов в install-хендлере
...
>   клиент-присланному `clientEndpoint` как host'у — деривит его из `domain` (грант-authoritative или
>   присланный). Сам `domain` install-хендлер валидирует по `isAllowedPortalDomain` (`*.bitrix24.<tld>`)
>   ДО исходящих REST → недопустимый домен = 400 `b24_install_bad_domain`, вызовы на внутренний URL закрыты.
