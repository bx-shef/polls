 ...
 *
 * ⚠ УГЛОВЫЕ СКОБКИ ТОЖЕ, и это не перестраховка. Запись едет делом, а `DESCRIPTION_TYPE`
 * проставляется ОТДЕЛЬНЫМ вызовом после создания. Что портал подставляет по умолчанию, когда
 * тип не передан, мы не проверяли — а среди возможных значений есть HTML (`3`). То есть
 * в окне между двумя вызовами, и в редком случае, когда компенсирующее удаление тоже не
 * сработало, `<img src=x onerror=…>` из ответа постороннего человека остался бы в чужой CRM
 * живой разметкой навсегда. Держать защиту на том, что чужой метод всегда отработает, нельзя.
 * Нашла панель ревью по PR #37–#39.
 *
