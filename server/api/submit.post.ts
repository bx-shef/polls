...
// по умолчанию — без этого /api/submit открыт для DoS большим телом. Здешняя проверка даёт
// ТОЧНЫЙ текст отказа («сократите ответ»); тело без заявленной длины (chunked) она пропустила бы
// как нулевое — этот случай закрывает общий бэкстоп `server/middleware/body-limit.ts` (411), до
