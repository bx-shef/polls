...
export function markerMatchesSurvey(originId: unknown, surveyKey: string): boolean {
  ...
  // ⚠️ Инвариант «в ключе перехода нет двоеточий» держат ДВА производителя, и при правке любого
  // сверяться надо здесь: `readTransitionId` (положительное целое) и `robotTransition`
  // (`robot-<целое>`). Появится третий, дающий двоеточие, — мы перестанем узнавать свои же дела, и
  // закрытие ответом умрёт молча.
