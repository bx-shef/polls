...
export default defineConfig({
  ...
  use: {
    ...
    // ⚠️ Аварийный путь к браузеру: `PW_CHROMIUM_PATH`. Playwright ищет ровно ту ревизию chromium,
    // которую пинит его версия, и в среде с ПРЕДУСТАНОВЛЕННЫМ браузером другой ревизии гейт падает
    // «Executable doesn't exist» ещё до первого снимка. Скачивать браузер в такой среде нельзя, а
    // молча пропускать гейт — хуже всего: правка UI уедет неувиденной.
    // ⚠️ Эталоны сверяются в ЗАКРЕПЛЁННОЙ среде: чужая ревизия рендерит иначе, и перегенерация под
    // неё испортит эталоны всем. Переменная нужна, чтобы УВИДЕТЬ экран и померить расхождение, а не
    // чтобы обновлять эталоны.
  ...
  expect: {
    toHaveScreenshot: {
      ...
      //
      // Измерено на закреплённой среде: прогон с НУЛЕВЫМ допуском давал все эталоны байт-в-байт (замер на 48; сейчас их 66),
