...
| Фронт-экраны (контур A) | Интро/Опрос/Спасибо (`/s/:key`, `useSurvey` поверх `SurveyFill` + `/api/*`) | 🔶 happy-path + гейт intro/survey/thanks/error + persist/deep-link ✅; загрузка/тёмная тема → #34 |
...
Визуальный гейт покрывает 4 поверхности (intro/survey/thanks/error=404) на 3 брейкпоинтах — через
управляющую навигацию Playwright. Дальше (#34): состояния загрузка/пусто/submit-ошибка, тёмная тема.
