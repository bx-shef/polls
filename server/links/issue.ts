...
/**
 * Погасить выпущенную ссылку.
 *
 * ⚠ ЭТО И ЕСТЬ ОТЗЫВ. Состояние элемента на портале — то, что видит менеджер; закрывает
 * страницу именно эта запись, потому что публичная страница в портал не ходит по инварианту
 * и решение о доступе принимает `decideLinkAccess` по нашей строке.
 *
 * ⚠ Снимок шапки СТИРАЕТСЯ вместе с отзывом. Там лежат имена людей — единственные
 * персональные данные в нашей схеме, — и держат их ровно затем, чтобы показать страницу.
 * Отозванная ссылка страницы больше не покажет, значит хранить их дальше нечем оправдать.
 * То же правило действует для пройденных (`markCompleted`) и для истёкших
 * (`forgetExpiredLinkHeaders`) — последнее с 25.09, до инвентаризации базы эта строка
 * обещала то, чего не было.
 *
 * ⚠ Гасим по паре «портал + элемент», а не по идентификатору строки: вызывающий пришёл
 * из карточки сделки и знает элемент смарт-процесса, а не нашу строку. Условие по порталу
 * обязательно — без него сотрудник одного портала гасил бы ссылку другого, совпади числа.
 *
 * Возвращает, сколько строк погашено: ноль значит «такой ссылки у нас нет» — например,
 * элемент создан не нами или строка уже стёрта. Это не ошибка, но и не успех.
 */
export async function revokeLink(portalId: string, itemId: number): Promise<number> {
  const revoked = await getDb()
    .update(schema.linkIndex)
    .set({ status: 'revoked', header: null })
    .where(and(
      eq(schema.linkIndex.portalId, portalId),
      eq(schema.linkIndex.itemId, itemId),
      // Пройденную ссылку не гасим: ответ клиента уже получен, и «отозвано» задним числом
      // означало бы, что его как будто не было.
      ne(schema.linkIndex.status, 'completed'),
    ))
