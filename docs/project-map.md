...
| `store/` | `IStore` + `MemoryStore` + `PgStore` (PostgreSQL, tenant-scoped) | ✅ | живой Postgres по `DATABASE_URL` (#6) |
...
| `server/` (Nitro) | обёртки ядра: session · submit · survey · health · dashboard · admin · b24/* | ✅ (dev-стор MemoryStore+seed) | PgStore-привязка (#6/#49) |
...
| Установка Bitrix24 | `/api/b24/install` (токены + робот + плейсменты) + handshake `/api/b24/session` | 🟡 работает | сквозной прогон после #6/#49 |
...
1. **#6/#49** — привязка Nitro к PgStore по `DATABASE_URL` (пул, `setPortalResolver`, tenant-фильтр).
   **Без этого данные не сохраняются между рестартами** и handshake фрейма fail-closed (401).
2. **#17** — авто-триггер `ONCRMDEALUPDATE` (эндпоинт + `event.bind`): ядро готово и маппер/формат
   стадий live-verified, нужен Nitro-эндпоинт + живой OAuth-round-trip.
3. **Живой install-smoke** Фазы A на тест-портале.
4. **Фаза B** edge-security · **#15** OTel · **#4** общий стор анти-абьюза.
...
### Сценарии применения (из брифа §13)
...
Собранное приложение сейчас на **MemoryStore + сид** (данные реалистичны, но **эфемерны** — сброс при
рестарте). Привязка PostgreSQL — **#6**. Готово к запуску **демо**; прод-хранение требует #6.
...
> ⛔ **Блокер сквозного прогона (#6 + #49):** `POST /api/b24/session` отвечает 401, пока резолвер
> `domain → member_id` пустой (no-op). Нужны живой PostgreSQL (`DATABASE_URL` → `setPortalResolver`) +
> заполненная таблица `portal`.

**Сегодня (без #6/#49):** демо целиком; рендер виджетов `/b24/deal-widget`·`task-widget`·`dashboard`;
`/api/health` за TLS → 200. **Полный прогон (после #6/#49):** установка (робот + плейсменты, сверить
...
| Установка → «проверка привязки портала не пройдена» (`403`) | authoritative `member_id` ≠ заявленному / `invalid_grant` (§2.3) | защита: чужой/подделанный грант отвергнут; легитимная — переустановить (свежий грант); лог `b24_install_member_reject` |
...
| Данные пропали после рестарта | работает MemoryStore (демо) | привязать PgStore (#6) |
...
- **48 эталонов** = 8 поверхностей (5 контур A: intro/survey/thanks/error/submit-error + 3 контур B:
  дашборд/фильтр-по-версии/дашборд-ошибка) × 3 брейкпоинта × 2 темы (light/dark) + 2 поведенческих
  теста без скриншота = **60 прогонов**. Admin-UI (`/admin/*`) пока НЕ в гейте.
...
- **Фаза B 🟠** — устойчивость деплоя: **#6/#49** привязка PgStore по `DATABASE_URL` (пул,
  `setPortalResolver`, tenant-фильтр) — **без неё данные не persist'ятся**; edge-security (CSP/HSTS/
...
- **Фаза D 🟠🟡** — **#17** авто-триггер `ONCRMDEALUPDATE` (эндпоинт + `event.bind`; маппер/формат стадий
  live-verified); **#18** результат → таймлайн сделки + просмотр/PDF; рейтинг приложения + виджет
...
- **Handshake фрейма** — обмен `BX24.getAuth` на подписанную сессию (cookie `polls_portal`); SSRF-allowlist
  + сверка `member_id`.
- **install-poisoning** — подделка установки с чужим `member_id`; **member_id-binding** — защита (§2.3).
